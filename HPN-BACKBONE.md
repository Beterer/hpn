...
- **Moderation restriction reflects into the feed via `profile.status`, driven by shared events.** The lifecycle events (`UserRestricted` / `UserBanned` / `UserCleared`) live in **`Hpn.SharedKernel.Moderation`**, not `Moderation.Contracts`, so Profile can subscribe without a Profile↔Moderation project cycle (same shape as `AccountDeletionRequested`). Profile moves the account to `under_review` / `banned` / back to `active`; eligibility already filters on `active`, so Feed needs no Moderation awareness. **`IModerationApi` is read-only** (`GetTrustScore`, `IsRestricted`); reports are filed via the HTTP `POST /reports` endpoint, not a cross-module contract call — writes stay in the module's slices/services, consistent with the other modules' read-only APIs (minor deviation from §6.7's listed `SubmitReport`). **Trust is computed on-demand from live cross-module signals (via Contracts) and cached in `account_trust` at the moderation-relevant moments** — a report on/by the user, or an action against them — rather than subscribing to every module's events, which keeps domain-event handlers fast (§10.7). **Restriction expiry is a gated maintenance step** (`RestrictionExpiryService.ReleaseExpiredAsync`), not a worker — same posture as the account purge; it records a `clear` action and raises `UserCleared`. The **reports rate-limit was corrected to a per-reporter partition** (was a global fixed window) per §10.6. (§M9)
